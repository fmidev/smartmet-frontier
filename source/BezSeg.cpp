...
void BezSeg::computeSegmentPointPositions()
{
  ...
  else
  {
    ...
    {
      // "Guess" curve tangent at the end of curve by assuming curve to run ahead as a straight
      // line.
      // This is implemented by the means of creating an extraneous (virtual) curve point.

      start = DoubleArr(curvePositions[startIndex].getCoordinates()[X],
                        curvePositions[startIndex].getCoordinates()[Y]);
      DoubleArr extraneousCurvePoint = Vector2Dee::getUnitVector(
          Vector2Dee::getSubtractedVector(start, curvePositions[startIndex - 1].getCoordinates()));
  ...
  start = DoubleArr(curvePositions[startIndex].getCoordinates()[X],
                    curvePositions[startIndex].getCoordinates()[Y]);
  end = DoubleArr(curvePositions[endIndex].getCoordinates()[X],
                  curvePositions[endIndex].getCoordinates()[Y]);

  // Create 1st control point

  b = DoubleArr(curvePositions[bStartIndex].getCoordinates()[X],
                curvePositions[bStartIndex].getCoordinates()[Y]);
  c = DoubleArr(curvePositions[cStartIndex].getCoordinates()[X],
                curvePositions[cStartIndex].getCoordinates()[Y]);
  ...
  {
    a = DoubleArr(curvePositions[aStartIndex].getCoordinates()[X],
                  curvePositions[aStartIndex].getCoordinates()[Y]);
  }
  ...
  a = DoubleArr(curvePositions[aEndIndex].getCoordinates()[X],
                curvePositions[aEndIndex].getCoordinates()[Y]);
  b = DoubleArr(curvePositions[bEndIndex].getCoordinates()[X],
                curvePositions[bEndIndex].getCoordinates()[Y]);

  if (cEndIndex >= (int)curvePositions.size())
  {
    // Approximate missing point 'c' at the end of curve
    DoubleArr extraneousCurvePoint =
        Vector2Dee::getUnitVector(Vector2Dee::getSubtractedVector(b, a));
    c = DoubleArr(b[X] + extraneousCurvePoint[X], b[Y] + extraneousCurvePoint[Y]);
  }
  else
  {
    c = DoubleArr(curvePositions[cEndIndex].getCoordinates()[X],
                  curvePositions[cEndIndex].getCoordinates()[Y]);
  }
...
{
  // Computes and returns a Bézier control point based on three given curve points and a tangent
  // direction value.
  ...
  DoubleArr m = Vector2Dee::getUnitVector(Vector2Dee::getAddedVector(
      Vector2Dee::getUnitVector(delta_a), Vector2Dee::getUnitVector(delta_c)));
...
DoubleArr BezSeg::getPosition(double t)
{
  ...
  if((this.segmentIndex == 0) || (this.segmentIndex == this.curvePositions.size()-1)) //ALKUP
  {
          ...
          return new GeneralDirectPosition(b0*this.start[X] + b1*this.c1.getCoordinates()[X] +
  b2*this.end[X],
                                                   b0*this.start[Y] + b1*this.c1.getCoordinates()[Y]
  + b2*this.end[Y]);


          // TESTII
          //return new
  GeneralDirectPosition(b0*curvePositions.get(this.segmentIndex).getCoordinates()[X] +
  b1*this.c1.getCoordinates()[X] + b2*curvePositions.get(this.segmentIndex+1).getCoordinates()[X],
                                                             b0*curvePositions.get(this.segmentIndex).getCoordinates()[Y]
  + b1*this.c1.getCoordinates()[Y] +
  b2*curvePositions.get(this.segmentIndex+1).getCoordinates()[Y]);

  }
  */

  // Evaluate *cubic* Bézier segment.
  // Returns evaluated t-point [0..1] on given Bézier segment:  P(t) = (1 - t)^3 * P0 + 3t(1-t)^2 *
  // P1 + 3t^2 (1-t) * P2 + t^3 * P3
...
//	double tEps = dt/2.;
//
//	// When necessary, make an artificial position displacement on base point just to make it
// possible to compute normal in a simplest way
//
//	double tDispaced = t;
...
//	DoubleArr normal = Vector2Dee.getUnitNormal(leftPosition, basePosition, rightPosition,
// isLeftOriented);
//
...
DoubleArr BezSeg::getTangent(double t)
{
  ...
  DoubleArr tang(-3. * t1 * t1 * start[X] + 3. * t1 * (1 - 3 * t) * c1[X] +
                     3. * t * (2 - 3 * t) * c2[X] + 3. * t * t * end[X],

                 -3. * t1 * t1 * start[Y] + 3. * t1 * (1 - 3 * t) * c1[Y] +
                     3. * t * (2 - 3 * t) * c2[Y] + 3. * t * t * end[Y]);
  ...
  ***********/
  ...
  /*********** OK	(ehkä paras?)
     ...
     P2 + 3t^2 * P3
          ...
          DirectPosition tang = new GeneralDirectPosition(-a1*start[X] + a1*c1[X] - a2*c1[X] -
     a3*c2[X] + a2*c2[X] + a3*end[X],
                                                                              -a1*start[Y] +
     a1*c1[Y] - a2*c1[Y] - a3*c2[Y] + a2*c2[Y] + a3*end[Y]);
