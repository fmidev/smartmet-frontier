...
Vector trisolve(const Vector& a, const Vector& b, const Vector& c, const Vector& rhs)
{
  if (b[0].x == 0 || b[0].y == 0)
    throw std::runtime_error(
        "Singular matrix encountered in PathFactory when converting spline representations");
...
Path create(const woml::CubicSplineRing& spline)
{
  ...
  Point fact = (x[0] + beta * x[n - 1] / gamma) / (1 + z[0] + beta * z[n - 1] / gamma);
  ...
  for (size_t i = 0; i < n; ++i)
  {
    ...
    // Working version
    path.curveto(
        x[i].x, x[i].y, 2 * knots[j].x - x[j].x, 2 * knots[j].y - x[j].y, knots[j].x, knots[j].y);
#else
    // Net version
    path.curveto(
        x[i].x, x[i].y, 2 * knots[i].x - x[i].x, 2 * knots[i].y - x[i].y, knots[j].x, knots[j].y);
#endif
  }
