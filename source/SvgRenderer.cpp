...
const double symbolBBoxFactorMin =
    0.5;  // Minimum symbol bbox 0.5 * symbol width/height; symbols can overlap
const size_t pathScalingSymbolHeightFactorMax =
    5;  // Surface scaling max offset 5 * (symbol height / 2)
const double textWitdhFactor =
...
class PathProjector : public PathTransformation
{
 public:
  ...
  void operator()(double &x, double &y, NFmiAngle *trueNorthAzimuth = nullptr) const
  {
    ...
    if (trueNorthAzimuth) *trueNorthAzimuth = area->TrueNorthAzimuth(ll);
  }
                  ...
                  double spacing)
{
  ...
  fronts << "<use class=\"" << lineclass << "\" xlink:href=\"#" << id << "\"/>\n";
  ...
  if (textlength > 0)
  {
    ...
    for (int j = 0; j < intervals; ++j)
    {
      ...
      fronts << " <text><textPath xlink:href=\"#" << id << "\" startOffset=\"" << std::fixed
             << std::setprecision(1) << offset << "\">" << (j % 2 == 0 ? glyphs1 : glyphs2)
             << "</textPath></text>\n";
    }
    fronts << "</g>\n";
  }
              ...
              const libconfig::Setting **matchingScope = nullptr)
{
  ...
  if ((!_isSet) && (typeid(T) != typeid(T2)))
  {
    ...
    if (_isSet) value = boost::lexical_cast<T>(value2);
  }
  ...
  if (settingId != s_required)
    throw SettingIdNotFoundException(settingId,
                                     "Setting for " + localScopeName + "." + param + " is missing");
              ...
              bool truncate = false)
{
  ...
  std::list<const libconfig::Setting *>::reverse_iterator rbegiter(scope.end()),
      renditer(scope.begin()), riter, it, git;
  ...
  for (riter = rbegiter; ((riter != renditer) && (!_isSet));)
  {
    ...
    if (riter != renditer) riter++;

    // For the last block(s) use the given settingId; the call throws if the setting is required but
    // not found
    //
  ...
  if (truncate && _isSet)
  {
    ...
    //
    std::list<const libconfig::Setting *>::iterator fenditer = scope.end(),
                                                    fiter =
                                                        ((matchingScope == *it) ? it : git).base();
...
FmiLanguage locale2FmiLanguage(const std::string &confPath, const std::string &theLocale)
{
...
template <typename T>
void SvgRenderer::render_aerodrome(const T &theFeature)
{
...
void SvgRenderer::render_elevationAxis()
{
  ...
  for (; (it != elevations.end()); it++)
  {
    ...
    if (!lLabel.empty())
      texts[ELEVATIONLABELS1] << "<text x=\"0\" y=\"" << std::fixed << std::setprecision(1)
                              << (axisHeight - it->scaledElevation()) << "\">" << lLabel
                              << "</text>\n";

    const std::string &rLabel = it->rLabel();
    if (!rLabel.empty())
      texts[ELEVATIONLABELS2] << "<text x=\"0\" y=\"" << std::fixed << std::setprecision(1)
                              << (axisHeight - it->scaledElevation()) << "\">" << rLabel
                              << "</text>\n";
  }
}
...
void SvgRenderer::render_timeAxis(const boost::posix_time::time_period &theTimePeriod)
{
  ...
  try
  {
    ...
    if (!timeSpecs.isGroup()) throw std::runtime_error(confPath + typeMsg);
    ...
    for (; (eit != elevations.end()); eit++)
      if (eit->line())
      {
        ...
        if (y > 0.1)
          texts["ELEVATIONLINES"] << "<line x1=\"0\" y1=\"" << std::fixed << std::setprecision(1)
                                  << (axisHeight - y) << "\" x2=\"" << std::fixed << width
                                  << "\" y2=\"" << std::fixed << std::setprecision(1)
                                  << (axisHeight - y) << "\"/>\n";
      }
  }
                                ...
                                const std::string &confPath)
{
  try
  {
    ...
    if (!hdrSpecs.isList()) throw std::runtime_error(confPath + typeMsg);
    ...
    // and no locale),
    // locale global blocks (blocks with matching name but having no locale and blocks having no
    // name
    ...
    for (int i = 0; i <= lastIdx; ++i)
    {
      ...
      if (!specs.isGroup()) throw std::runtime_error(confPath + typeMsg);
      ...
      if (nameMatch || noName || ((i == lastIdx) && hasLocaleGlobals))
      {
        if (nameMatch || noName)
        {
          ...
          // Currently no locale settings are required
          //
          //					if ((noName || (!localeMatch)) && ((i < lastIdx) ||
          //(!hasLocaleGlobals)))
          if ((noName || ((!localeMatch) && (locale != ""))) &&
        ...
        if (type.empty() || (type == "datetime"))
        {
          ...
          std::string pref(configValue<std::string>(scope, hdrClass, "pref")), prefDate,
              &format = pref;
          ...
          if (useDate)
          {
            ...
            if (!prefDate.empty()) format = prefDate;
          }
          ...
          texts[HEADERhdrClass]
              << (datetime.is_not_a_date_time()
                      ? ""
                      : svgescapetext(
                            toFormattedString(datetime, format, isSet && utc, language).CharPtr(),
                            true));
    ...
    if (options.debug)
    {
      ...
      debugoutput << p << hdrClass << " not found\n";
    }
  }
  catch (libconfig::SettingNotFoundException &ex)
  {
    ...
    //
    if (options.debug)
      debugoutput << "Settings for " << confPath << " (" << hdrClass << ") not found\n";
  }
}
...

{
  ...
  float symhoff[] = {
      (xoffset + shoffset), (xoffset - shoffset), (xoffset - shoffset), (xoffset + shoffset)};
  float symvoff[] = {
      (yoffset + svoffset), (yoffset - svoffset), (yoffset + svoffset), (yoffset - svoffset)};
  ...
  for (h = 0, x = hoffset; (h < nh); h++, x += (bw + hoffset))
    for (v = 0, y = voffset; (v < nv); v++, y += (bh + voffset), symCnt++)
      fpos.push_back(Point(iter->first.x + x + symhoff[symCnt % shoCnt],
                           iter->first.y + y + symvoff[symCnt % svoCnt]));
...
{
  if (area1.first.x < area2.first.x) return true;
  ...
  for (; (iter != areas.end()); iter++)
  {
    if ((iter->second.x - iter->first.x) > (2 * scaledWidth))
    {
      ...
      do
      {
        ...
        if ((x2 + floor(scaledWidth + 0.5)) > xEnd) x2 = xEnd;
...
bool sortFillAreaColumnAsc(const NFmiFillRect &r1, const NFmiFillRect &r2)
{
  return (r1.first.y < r2.first.y);
}
bool sortFillAreaColumnDesc(const NFmiFillRect &r1, const NFmiFillRect &r2)
{
  return (r1.first.y > r2.first.y);
}
...
struct Column
{
  Column(NFmiFillAreas::const_iterator fstArea, size_t n, bool sortAsc = true)
  {
    ...
    if (n > 1) fillAreas.sort(sortAsc ? sortFillAreaColumnAsc : sortFillAreaColumnDesc);
  }
...
{
  // Extract fill area "columns", groups of subsequent fill areas having starting x -coordinate less
  // than or
...
void getFillPositions(NFmiFillAreas &areas,
                      size_t symbolCnt,
                      NFmiFillPositions &fpos,
                      const std::string &fillMode = "")
{
  // Extract fill area "columns", groups of fill areas having starting x -coordinate greater than or
  // equal to
  // the starting x -coordinate and less than or equal to the center x -coordinate of group's first
  // fill area.
  ...
  else if (fillMode == "column")
  {
    ...
    //
    if (columns.size() >= symbolCnt) symbolCnt = columns.size();
  }
  else if ((strlen(fillMode.c_str()) >= 2) && (strlen(fillMode.c_str()) <= 4) &&
           (strspn(fillMode.c_str(), "1234567890") == (strlen(fillMode.c_str()) - 1)) &&
           strrchr(fillMode.c_str(), '%') && (atoi(fillMode.c_str()) <= 100))
  {
    ...
    if (nSym > symbolCnt) symbolCnt = std::min(nSym, nPos);
  }
  else
    throw std::runtime_error("getFillPositions: invalid fill mode: '" + fillMode + "'");

  size_t nAreasLeft = areas.size(), nSymbolsLeft = symbolCnt, nColsLeft = columns.size(),
         colStepN = 1, colStep = 1, col, n;
  ...
  for (FillAreaColumns::const_iterator iCol = columns.begin();
       ((iCol != columns.end()) && (nSymbolsLeft > 0));
       iCol++, nColsLeft--)
  {
    // Step over columns
    //
    ...
    if ((nColSymbols * nColsLeft) < nSymbolsLeft) nColSymbols++;
    ...
    for (col = 0, n = 1; ((col < iCol->nRows()) && (nColSymbols > 0) && (nSymbolsLeft > 0));
         col++, iArea++)
    {
      if (n >= rowStep)
      {
  ...
  if (nSymbolsLeft > 0) throw std::runtime_error("getFillPositions: unexpected end of columns");
}
                      ...
                      const std::string &fillMode = "")
{
  ...
  for (NFmiFillAreas::const_iterator iter = areas.begin();
       ((iter != areas.end()) && (nSymbolsLeft > 0));
       nSymbolsLeft--)
  {
    double xOff, yOff;
    ...
    else
    {
      ...
      if (symIndex <= (4 - maxAreaSymbols)) symIndex = 4;
    }
  ...
  if (nSymbolsLeft > 0) throw std::runtime_error("getFillPositions: unexpected end of fill areas");
}
...
{
  if (areas.size() < 1) throw std::runtime_error("getCenterFillAreaRect: no fill areas");
  ...
  if (colStep > 1) advance(iCol, colStep - 1);
  ...
  if (rowStep > 1) advance(iArea, rowStep - 1);
                  ...
                  unsigned int &maxLineHeight)          // Max line height
{
  ...
  for (l = 0; (l < textlc); l++)
  {
    ...
    unsigned int lineWidth =
        static_cast<unsigned int>(ceil(std::max(extents.width, extents.x_advance)));
    ...
    for (w = 0; (w < textwc);)
    {
      ...
      if (!(fits = (ceil(extents.width) < maxWidth)))
      {
        ...
        if (pos != std::string::npos)
        {
          ...
          if ((fits = (ceil(extents.width) < maxWidth))) word = cutted;
        }
      }
      ...
      //			if ((maxHeight > 0) && (maxHeight < ((outputLines.size() *
      //(maxLineHeight + 2)) - 2)))
      //				// Max height exceeded; decrement font size (downto half of
      // the given size) and regenerate the text
      //				;
  ...
  //		// Max height exceeded; decrement font size (downto half of the given size) and
  // regenerate the text
  //		;
  //
  //  }  // for regenerate with smaller fontsize
                              ...
                              int *tYOffset)      // I/O: text's y -offset
{
  try
  {
    ...
    // Note: 'textName' contains area/symbol type for area/symbol infotexts; using confPath to
    // generate fixed placeholder names.
    //		 'useTextName' is set when rendering cloudlayer, icing/turbulence and zerotolerance
    // labels; using the text name
    //		  as text output placeholder as is.

    std::string TEXTCLASStextName("TEXTCLASS" +
                                  ((TEXTPOSid.empty() || useTextName) ? textName : confPath));
    std::string TEXTtextName(useTextName ? textName
                                         : ("TEXT" + (TEXTPOSid.empty() ? textName : confPath)));
    ...
    if (!textSpecs.isList()) throw std::runtime_error(confPath + typeMsg);
        ...
        if (nameMatch || noName || ((i == lastIdx) && hasLocaleGlobals))
        {
          if (nameMatch || noName)
          {
            ...
            if (localeMatch || (locale == ""))
            {
              ...
              if ((nameMatch && (locale == "")) || (noName && localeMatch)) hasLocaleGlobals = true;
            }

            if ((noName || (!localeMatch)) && ((i < lastIdx) || (!hasLocaleGlobals))) continue;
          }
          ...
          //
          // For area/symbol text (when tXOffset and tYOffset are passed in as nonnull) the offsets
          // are used later
          // when setting the final text position.
          //
          // Note: If the passed offsets (or font size) are nonzero, they were set in the text and
          // won't be overridden by config settings.
          unsigned int maxWidth = ((maxTextWidth && (*maxTextWidth > 0))
                                       ? *maxTextWidth
                                       : configValue<unsigned int>(scope, textName, "textwidth"));

          bool isSet;
          unsigned int maxHeight =
              configValue<unsigned int>(scope, textName, "textheight", s_optional, &isSet);
          if (!isSet) maxHeight = 0;

          unsigned int margin =
              configValue<unsigned int>(scope, textName, "margin", s_optional, &isSet);
          if (!isSet) margin = 2;

          if (maxWidth <= 20) maxWidth = 20;
          if ((maxHeight != 0) && (maxHeight <= 20)) maxHeight = 20;

          int xOffset = configValue<int>(scope, textName, "txoffset", s_optional, &isSet);
          if ((!isSet) || tXOffset)
          {
            if (isSet && (*tXOffset == 0)) *tXOffset = xOffset;
          ...
          if ((!isSet) || tYOffset)
          {
            if (isSet && (*tYOffset == 0)) *tYOffset = yOffset;
          ...
          std::string textClass("text" + ((TEXTPOSid.empty() || useTextName)
                                              ? textName
                                              : TEXTPOSid.substr(TEXTPOSid.find("_") + 1)));
          ...
          for (x += margin, y += margin; (it != textLines.end()); it++)
          {
            ...
            if (it == textLines.begin())
            {
              if (!TEXTPOSid.empty())
                ...
                //
                // Note: Scaling placeholder/value might not get set; unset placehoders will be
                // removed prior output
                //
              ...
              if (!bStyle.empty())
                ...
                texts[TEXTtextName] << "<rect width=\"" << textWidth - (2 * bXOffset)
                                    << "\" height=\"" << textHeight - (2 * bYOffset) << "\" x=\""
                                    << bXOffset << "\" y=\"" << bYOffset << "\" style=\"" << bStyle
                                    << "\""
                                    << "/>";
            }

            texts[TEXTtextName] << "<text class=\"" << textClass << "\" x=\"" << x << "\" y=\"" << y
    ...
    if (options.debug)
    {
      ...
      debugoutput << p << textName << " not found\n";
    }
  }
  catch (libconfig::SettingNotFoundException &ex)
  {
    ...
    //
    if (options.debug)
      debugoutput << "Settings for " << confPath << " (" << textName << ") not found\n";
  }
}
                   ...
                   std::list<const libconfig::Setting *> *scope = nullptr)
{
  ...
  //		 have their '+' and '@' characters replaced with 'P_' and '_' respectively (e.g.
  //'+RA'
  // is converted to 'P_RA')
  ...
  if (!mappedCode.empty())
  {
    ...
    if (!symbolSpecs.isGroup()) throw std::runtime_error(confPath + typeMsg);
...
void textSettings(std::string &infoText,
                  ...
                  int &tYOffset)
{
  // The infotext can start with text position (<area> (or shortly <a>), <top> etc.), max text width
  // (<width=n>),
  // font size (<fontsize=n>) and x/y offset (<x/yoffset=n>) settings.

  const char *settings[] = {
      // long name		short name
      "<area>",       "<a>",  "<center>",      "<c>",  "<top>",   "<t>", "<topleft>", "<tl>",
  ...
  for (const char **s = settings, **ps = nullptr; *s;)
    if (infoText.find(*s) == 0)
    {
      ...
      if (*(*s + strlen(*s) - 1) == '=')
      {
        ...
        const char *p2 =
            p + strspn(p, numSet + (((*(*s + 1) == 'w') || (*(*s + 1) == 'f')) ? 1 : 0));

        if ((*p2 != '>') || (*(p2 - 1) == '=') || (*(p2 - 1) == '-')) return;

        int &setting =
            ((*(*s + 1) == 'w')
                 ? maxTextWidth
                 : ((*(*s + 1) == 'f') ? fontSize : ((*(*s + 1) == 'x') ? tXOffset : tYOffset)));
                                  ...
                                  Path::BBox *boundingBox)
{
  ...
  if ((textPosition == "top") || (textPosition == "bottom") || (textPosition == "topleft") ||
      (textPosition == "topright") || (textPosition == "bottomleft") ||
      (textPosition == "bottomright"))
  {
    if ((textPosition == "top") || (textPosition == "bottom"))
      x = bbox.blX + (bboxWidth / 2) - (textWidth / 2) + tXOffset;
    else
      x = ((((textPosition == "topleft") || (textPosition == "bottomleft")) ? bbox.blX : bbox.trX) -
           (textWidth / 2)) +
          tXOffset;

    y = ((textPosition.find("top") == 0) ? (bbox.trY - textHeight - tYOffset)
                                         : (bbox.blY + tYOffset));
  }
  else if ((textPosition == "left") || (textPosition == "right"))
  {
    ...
    throw std::runtime_error("invalid text position: '" + textPosition +
                             "'; use top[left|right], bottom[left|right], left or right");
...
void SvgRenderer::render_surface(
    Path &path,
    std::ostringstream &surfaceOutput,
    const std::string &id,
    const std::string &surfaceName,  // cloudType (for CloudArea), rainPhase (for
                                     // SurfacePrecipitationArea or area type for
                                     // ParameterValueSetArea)
    const woml::Feature *feature,    // For area's infotext rendering
    const std::list<std::string>
        *areaSymbols)  // Symbols to be placed into ParameterValueSetArea (warning area's symbols)
{
  ...
  try
  {
    ...
    if (!surfSpecs.isList()) throw std::runtime_error(confPath + typeMsg);
    ...
    for (int i = 0; i <= lastIdx; ++i)
    {
      ...
      if (nameMatch || noName || ((i == lastIdx) && (!scope.empty())))
      {
        if (nameMatch || noName)
        {
          if (nameMatch) surfIdx = i;

          scope.push_back(&specs);

          if (noName && (i < lastIdx)) continue;
        }
        ...
        if ((type == "pattern") || (type == "glyph") || filled || masked)
        {
          ...
          //
          std::string classDef(
              (type != "pattern") ? configValue<std::string>(scope, surfaceName, "class") : "");
          ...
          std::string placeHolder(boost::algorithm::trim_copy(
              configValue<std::string>(scope, surfaceName, "output", s_optional)));
          std::ostringstream &surfaces = (placeHolder.empty() ? surfaceOutput : texts[placeHolder]);

          if (type == "pattern")
            surfaces << "<g id=\"" << id << "\">\n"
                     << "<path style=\"" << style << "\" d=\"" << path.svg() << "\"/>\n"
                     << "</g>\n";
          else
          {
            std::string pathId(id + type);

            paths << "<path id=\"" << pathId << "\" d=\"" << path.svg() << "\"/>\n";

            surfaces << "<use class=\"" << classDef << "\" xlink:href=\"#" << pathId
                     << ((filled && (!masked)) ? "\"/>\n" : "");
            ...
            if (masked)
            {
              ...
              masks << "<mask id=\"" << maskId << "\" maskUnits=\"userSpaceOnUse\">\n"
                    << "<use xlink:href=\"#" << pathId << "\" class=\"" << style
                    << "\"/>\n</mask>\n";

              surfaces << "\" mask=\"url(#" << maskId << ")\"/>\n";
            }
            ...
            if (filled)
            {
              ...
              bool isSet;
              bool autoScale =
                  configValue<bool>(scope, surfaceName, "autoscale", s_optional, &isSet);
              if (!isSet) autoScale = false;

              // If mode is 'vertical' (default: horizontal) the symbols are placed using y -axis as
              // the
              // primary axis (using maximum vertical fillrects instead of maximum horizontal
              // fillrects)
              std::string mode = configValue<std::string>(scope, surfaceName, "mode", s_optional);
              bool verticalRects = (mode == "vertical");

              // If direction is 'both' (default: up) iterating vertically over the surface
              // y-coordinates
              // from both directions; otherwise from bottom up
              std::string direction =
                  configValue<std::string>(scope, surfaceName, "direction", s_optional);
              bool scanUpDown = (direction == "both");

              // If showareas is true (default: false) output the fill area bounding boxes too
              bool showAreas =
                  configValue<bool>(scope, surfaceName, "showareas", s_optional, &isSet);
              if (!isSet) showAreas = false;
              ...
              {
                // Render feature's infotext. The text is rendered starting from coordinate (0,0)
                // and
                // final position is selected afterwards. The final position is set as
                // transformation
                // offsets to the selected position.
                //
                // Note: The key for position must sort after the key for the text (the key/text
                //		 containing the position key must be handled/outputted prior the
                // position key/text);
                ...
                textPosition =
                    configValue<std::string>(scope, surfaceName, "textposition", s_optional);

                // The text can start with position, fontsize and x/y offset settings overriding the
                // configured values.
                ...
                render_text(texts,
                            confPath,
                            surfaceName,
                            NFmiStringTools::UrlDecode(textOut),
                            textWidth,
                            textHeight,
                            true,
                            false,
                ...
                if (textPosition == "area")
                {
                  ...
                  if (fmap.getFillAreas(areaWidth,
                                        areaHeight,
                                        textWidth,
                                        textHeight,
                                        1.0,
              ...
              // infotext into the area, by using smaller symbol bbox and by scaling the surface
              // bigger
              //
              NFmiFillMap fmap;
              NFmiFillAreas areas;
              ...
              if ((scale >= 1) && autoScale)
              {
                ...
                if ((isScaled = path.scale(
                         std::min(pathScalingOffset, (pathScalingSymbolHeightFactorMax * height)),
                         path)))
                {
              ...
              do
              {
                do
                {
                  ...
                  ok = sizeOk = fmap.getFillAreas(areaWidth,
                                                  areaHeight,
                                                  width,
                                                  height,
                                                  scale,
                  ...
                  {
                    // Check there is room for all area symbols. Erase fill areas overlapping the
                    // area
                    // reserved for info text.
                    //
                    // Note: To get fillareas with width near or equal to twice (etc) the symbol
                    // width splitted, decreasing scale
                    ...
                    for (iter = areas.begin(), fpos.clear();
                         ((iter != areas.end()) && (fpos.size() < fillSymbols.size()));)
                  ...
                  if (!ok)
                  {
                    ...
                    sizeOk = ((nw >= floor(_width * markerScaleFactorMin)) &&
                              (nh >= floor(_height * markerScaleFactorMin)));
                    noTextRetry = false;
              ...
              } while (autoScale && (!ok) && isScaled &&
                       (pathScalingOffset <= (pathScalingSymbolHeightFactorMax * height)));

              if (!ok)
              {
                if (!autoScale) throw std::runtime_error("render_surface: no space for symbol(s)");
                ...
                if ((fillSymbols.size() == 0) || ((4 * fpos.size()) < fillSymbols.size()))
                {
                  ...
                  double cx = bbox.blX + ((bbox.trX - bbox.blX) / 2),
                         cy = bbox.blY + ((bbox.trY - bbox.blY) / 2), x0, y0;
                  const int spacing = 2;
                  int nx, ny, ns;
              ...
              {
                for (NFmiFillAreas::const_iterator iter = areas.begin(); (iter != areas.end());
                     iter++)
                {
                  ...
                  if (showAreas)
                  {
                    ...
                    //
                    surfaces << "<rect x=\"" << iter->first.x << "\" y=\"" << iter->first.y
                             << "\" width=\"" << iter->second.x - iter->first.x << "\" height=\""
                             << iter->second.y - iter->first.y << "\" fill=\""
                             << clrs[clrIdx % clrCnt] << "\"/>\n";
                    clrIdx++;
                  }
                }
              ...
              {
                // fillMode controls whether to get one position for each fill symbol (the default)
                // or
                // for each column, or to get all or given percentage of the available positions
                //
                std::string fillMode =
                    configValue<std::string>(scope, surfaceName, "fillmode", s_optional);

                getFillPositions(
                    areas, width, height, fillSymbols.size(), scale, infoTextRect, fpos, fillMode);
    ...
    if (options.debug)
    {
      ...
      debugoutput << p << surfaceName << " not found\n";
    }
  }
  catch (libconfig::SettingNotFoundException &ex)
  {
    ...
    //
    if (options.debug)
      debugoutput << "Settings for " << confPath << " (" << surfaceName << ") not found\n";
  }
}
                                            ...
                                            const double numericValue)
{
  ...
  for (int i = 0; i < condSpecs.getLength(); i++)
  {
    ...
    try
    {
      ...
      if (badNan) throw std::runtime_error(confPath + ".conditions" + valMsg);
    ...
    {
      if (((condValue >= numericValue) || (fabs(condValue - numericValue) <= eps)) &&
  ...
  {
    idx = ((ltCondIdx < 0) ||
           ((gtCondIdx >= 0) && ((numericValue - ltCondValue) > (gtCondValue - numericValue))))
              ? gtCondIdx
              : ltCondIdx;
        ...
        if ((loRng.size() <= 2) || (hiRng.size() <= 2)) try
          {
            ...
            if ((lh <= 23) && (lm <= 59) && (hh <= 23) && (hm <= 59))
            {
              ...
              if ((vdt >= ldt) && (vdt <= hdt))
              {
                ...
                if (parameter.empty() || config.exists(condPath)) return &(condSpecs[i]);
              }
                               ...
                               bool commonCode = false)
{
  ...
  //
  // If commonCode is set, using primarily plain 'code' key instead given symCode (containing
  // code_<symCode>);
  ...
  if (cols.size() == 2) boost::trim(cols[codeIdx = 1]);
                           ...
                           libconfig::Setting *globalScope = nullptr)
{
  ...
  try
  {
    if (upperLimit)
    {
      ...
      //
      const woml::NumericalSingleValueMeasure *v1 =
          (((lowerLimit->numericValue() <= upperLimit->numericValue()) &&
            ((upperLimit->numericValue() >= 0)))
               ? lowerLimit
               : upperLimit);
      ...
      if (specs)
      {
        ...
        //
        // Note: The first value formatter is used for both range limit values when checking the
        // output value sign and if it is zero;
        //		 e.g. format '%.0f .. %.1f' would be taken as 'negtozero' instead of
        //'negtopos' for range -2..0.2 (the hi range
        //		 value would be formatted with '%.0f' instead of '%.1f'). This does not
        // matter
        // though because both range values
        //		 (currently temperature ranges only) very likely have similar formatter and
        // no
        // fractional part.
        //
        ...
        defaultOrder =
            configValue<bool>(*specs, "rangeType", rangeType, globalScope, s_optional, &isSet);
                                         ...
                                         const std::string symbolPosIdN)
{
  ...
  try
  {
    ...
    if (codeValue)
      texts[symClass] << "<g id=\"" << id << "\" class=\"" << class1 << class2 << "\" transform=\""
                      << transf.str() << "\">\n"
                      << "<use xlink:href=\"" << uri << "\"/>\n"
                      << "</g>\n";
    else
      texts[symClass] << "<g id=\"" << id << "\" class=\"" << class1 << "\" transform=\""
                      << transf.str() << "\">\n"
                      << "<text>" << value << "</text>\n"
                      << "</g>\n";
                                          ...
                                          bool setVisibility)
{
  ...
  try
  {
    ...
    const std::string symClass(
        boost::algorithm::to_upper_copy(confPath + theFeature.classNameExt()));
    ...
    for (iteg = eGrp.begin(); (iteg != egend); iteg++)
    {
      ...
      if ((!cvm) && (!nsv))
        throw std::runtime_error(
            ...
            ": CategoryValueMeasure or NumericalSingleValueMeasure values expected");

      if (!ground)
      {
        // Note: For "nonground" elevations (MigratoryBirds) checking for conditional y -position
        // based on the value (category);
        //		 used to position migration type (text 'morning', 'arctic' etc. as symbol)
        // relatively to the bird symbol or to
        //		 fixed y -position (above or below the x -axis).
        //
        //		 Conditions are expected to exist only for the texts, not for the actual
        // bird
        // symbols;
        ...
        if ((!condSpecs) && ((y > 0) && (y < axisManager->axisHeight()))) visible = true;
      ...
      else
      {
        ...
        const libconfig::Setting *condSpecs =
            matchingCondition(config, confPath, confPath, "", -1, value);
                                ...
                                int height)
{
  ...
  try
  {
    ...
    const libconfig::Setting &symbolSpecs = config.lookup(confPath);
    if (!symbolSpecs.isList()) throw std::runtime_error(confPath + typeMsg);
      ...
      if (nameMatch || ((i == lastIdx) && _hasLocaleGlobals))
      {
        if (nameMatch)
        {
          ...
          if (localeMatch || (locale == ""))
          {
            ...
            // Search for conditional settings based on the documents validTime (it's time value) or
            // given measurement value

            const libconfig::Setting *condSpecs =
                svm ? matchingCondition(
                          config, confPath, symClass, "", symbolIdx, svm->numericValue())
                    : matchingCondition(config, confPath, symClass, _symCode, symbolIdx, validtime);

            if (condSpecs) scope.push_back(condSpecs);
          }
          ...
          if ((!localeMatch) && ((i < lastIdx) || (!_hasLocaleGlobals))) continue;
        }
        ...
        if (!areaSymbols)
        {
          try
          {
            ...
            // nan)
            //
            code = getFolderAndSymbol(config,
                                      &scope,
                                      confPath,
                                      symClass,
                                      _symCode,
          ...
          catch (SettingIdNotFoundException &ex)
          {
            // Symbol code not found
            //
            // Clear _hasLocaleGlobals to indicate the settings have been scanned upto current block
            // and
            // there's no use to enter here again unless new locale global block(s) or current block
            // is met
            //
        ...
        else
        {
          ...
          xoffset = static_cast<int>(
              floor(configValue<float, int>(scope, symClass, "xoffset", s_optional, &isSet)));
          if (!isSet) xoffset = 0;
          yoffset = static_cast<int>(
              floor(configValue<float, int>(scope, symClass, "yoffset", s_optional, &isSet)));
          if (!isSet) yoffset = 0;
        }

        // Output placeholder; by default output to passed stream. The setting is ignored for
        // ParameterValueSetArea.

        std::string placeHolder(areaSymbols ? ""
                                            : boost::algorithm::trim_copy(configValue<std::string>(
                                                  scope, symClass, "output", s_optional)));
        ...
        if ((type == "svg") || (type == "img"))
        {
          ...
          std::string uri =
              configValue<std::string>(scope, symClass, "url", fpos ? s_required : s_optional);

          if (uri.empty())
          {
            std::string classDef = configValue<std::string>(scope, symClass, "class", s_optional);
            if (classDef.empty()) classDef = code;
            ...
            if (hasScale || trueNorthAdjustment)
            {
              ...
              if (hasScale)
                wh << "translate(" << -lon * (scale - 1) << "," << -lat * (scale - 1) << ") scale("
                   << scale << ")";
          ...
          if (!fpos)
          {
            ...
            {
              if (trueNorthAdjustment)
                symbols << "<g transform=\"rotate(" << trueNorthAzimuth.Value() << " " << lon << " "
                        << lat << ")\">\n";

              symbols << "<image xlink:href=\"" << svgescape(uri) << "\" x=\"" << std::fixed
                      << std::setprecision(1) << ((lon - width / 2) - xoffset) << "\" y=\""
                      << std::fixed << std::setprecision(1) << ((lat - height / 2) + yoffset)
                      << "\"" << wh.str() << "/>\n";

              if (trueNorthAdjustment) symbols << "</g>\n";
            }
          }
          else
          {
            ...
            if (areaSymbols) siter = areaSymbols->begin();

            for (piter = fpos->begin(); (piter != fpos->end()); piter++)
              if (areaSymbols)
              {
                if (siter == areaSymbols->end()) siter = areaSymbols->begin();
                ...
                if (siter != areaSymbols->end())
                {
                  ...
                  symbols << "<image xlink:href=\"" << svgescape(u) << "\" x=\"" << std::fixed
                          << std::setprecision(1) << (piter->x - (width / 2)) << "\" y=\""
        ...
        else if (type == "font")
        {
          ...
          if (!fpos)
            symbols << "<text class=\"" << class1 << class2 << "\" id=\"" << id << "\" x=\""
                    << std::fixed << std::setprecision(1) << (lon - xoffset) << "\" y=\""
                    << std::fixed << std::setprecision(1) << (lat + yoffset) << "\">&#" << code
                    << ";</text>\n";
          else
          {
            NFmiFillPositions::const_iterator piter;
            ...
            for (piter = fpos->begin(); (piter != fpos->end()); piter++)
              symbols << "<text class=\"" << class1 << class2 << "\" id=\"" << id << "\" x=\""
                      << std::fixed << std::setprecision(1) << piter->x << "\" y=\"" << std::fixed
                      << std::setprecision(1) << piter->y << "\">&#" << code << ";</text>\n";
          }
        }
        else
        ...
        if (feature)
        {
          ...
          if ((!textOut.empty()) && (textOut != options.locale))
          {
            ...
            //
            // Note: The key for position must sort after the key for the text (the key/text
            //		 containing the position key must be handled/outputted prior the position
            // key/text);
            //		 therefore the position key starts with "Z0"
            //
            ...
            int textWidth = 0, textHeight = 0, maxTextWidth = 0, fontSize = 0, tXOffset = 0,
                tYOffset = 0;
            int areaWidth = static_cast<int>(std::floor(0.5 + area->Width())),
                areaHeight = static_cast<int>(std::floor(0.5 + area->Height()));
            ...
            NFmiFillRect infoTextRect(std::make_pair(
                Point(lon - (textWidth / 2.0), lat - (textHeight / 2.0)), Point(0, 0)));
    ...
    if (options.debug)
    {
      ...
      //
      const char *p = (((symbolIdx < 0) || hasLocaleGlobals || (localeIdx >= 0))
                           ? "Settings for "
                           : "Locale specific settings for ");

      debugoutput << p << symClass << " (" << _symCode << ") not found\n";
    }
  }
  catch (libconfig::SettingNotFoundException &ex)
  {
    ...
    //
    if (options.debug)
      debugoutput << "Settings for " << confPath << " (" << symClass << symCode << ") not found\n";
  }
}
...
SvgRenderer::Phase SvgRenderer::uprightdown(
    ElevGrp &eGrp, ElevGrp::iterator &iteg, double lo, double hi, bool nonGndFwd2Gnd)
{
  ...
  for (riteg++; (riteg != eGrp.end()); riteg++)
  {
    ...
    else if (dh == 1)
    {
      ...
      // If nonGndFwd2Gnd is false (ZeroTolerance), can always go forward from below 0 to below 0 or
      // ground elevation
  ...
  if (upopen && ((!(uiteg->topConnected())) || (triteg == iteg)))
  {
    ...
    if (triteg != iteg) triteg->leftOpen(false);
    iteg = uiteg;
...
SvgRenderer::Phase SvgRenderer::downleftup(
  ...
  if (iteg != eGrp.begin())
  {
    ...
    if ((diteg != eGrp.end()) && (diteg->validTime() == vt) && (!(diteg->topConnected())))
    {
      ...
      if (iteg->bottomConnected())
      {
        ...
        {
          // Check if the elevation below overlaps the elevation on the right side of the current
          // elevation
          //
    ...
    for (;;)
    {
      ...
      else if (dh == -1)
      {
        ...
        // If nonGndVdn2Gnd is true (ZeroTolerance), can always go backward between below 0 and/or
        // ground elevations
        // (even if the elevations do not overlap)
        ...
        if (((llo <= hi) && (lhi >= lo)) || BWD)
        {
          ...
          // Select the lowest overlapping left side elevation for which the path has not travelled
          // by left side
                                                        ...
                                                        std::set<size_t> &cloudSet)
{
  ...
  try
  {
    ...
    // If bordercompensation is true (the default), cloud baseline is adjusted to keep decorated
    ...
    // If bbcenterlabel is true (default: false), cloud label is positioned to the center of the
    ...
    if (!groupSpecs.isList()) throw std::runtime_error(confPath + listtypeMsg);

    for (int i = 0; i < groupSpecs.getLength(); i++)
    {
      const libconfig::Setting &group = groupSpecs[i];
      if (!group.isGroup()) throw std::runtime_error(confPath + listtypeMsg);
      ...
      std::string placeHolder(boost::algorithm::trim_copy(
          configValue<std::string>(scope, confPath, "output", s_optional)));
      if (placeHolder.empty()) placeHolder = CLOUDLAYERS;

      std::string labelPlaceHolder(boost::algorithm::trim_copy(
          configValue<std::string>(scope, confPath, "labeloutput", s_optional)));
      if (labelPlaceHolder.empty()) labelPlaceHolder = placeHolder + "TEXT";
      ...
      if (!isSet) combined = false;

      // Render as symbol ?

      std::string symbolType(boost::algorithm::trim_copy(
          configValue<std::string>(scope, confPath, "symboltype", s_optional)));

      // Controls for extracting Bezier curve points; base distance between curve points along the
      // line,
      // max random distance added to the base and max number of subsequent points having equal
      // distance

      unsigned int baseStep =
          configValue<unsigned int>(scope, confPath, "baseStep", s_optional, &isSet);
      if (!isSet) baseStep = 10;
      unsigned int maxRand =
          configValue<unsigned int>(scope, confPath, "maxRand", s_optional, &isSet);
      if (!isSet) maxRand = 4;
      unsigned int maxRepeat =
          configValue<unsigned int>(scope, confPath, "maxRepeat", s_optional, &isSet);
      if (!isSet) maxRepeat = 3;

      // Controls for decorating the Bezier curve; base curve distance (normal length) for the
      // decorator points,
      // max random distance added to the base, base offset for the decorator points and max random
      // offset added to the base

      int scaleHeightMin = configValue<int>(scope, confPath, "scaleHeightMin", s_optional, &isSet);
      if (!isSet) scaleHeightMin = 5;
      int scaleHeightRandom =
          configValue<int>(scope, confPath, "scaleHeightRandom", s_optional, &isSet);
      if (!isSet) scaleHeightRandom = 3;
      int controlMin = configValue<int>(scope, confPath, "controlMin", s_optional, &isSet);
      if (!isSet) controlMin = -2;
      int controlRandom = configValue<int>(scope, confPath, "controlRandom", s_optional, &isSet);
      if (!isSet) controlRandom = 4;
      ...
      double yOffset =
          (double)configValue<int, double>(scope, confPath, "yoffset", s_optional, &isSet);
      ...
      // Relative top/bottom and side x -offsets for intermediate curve points for single elevation
                                     ...
                                     double hipx)
{
  ...
  texts[cg.placeHolder()] << "<g id=\"CloudLayers" << nGroups << "\" transform=\"translate("
                          << std::fixed << std::setprecision(3) << x << "," << (lopx - (hpx / 2))
                          << ")\">\n"
                        ...
                        bool eraseReserved = true)
{
       ...
       riter++)
  {
    ...
    {
      if ((markerId != reserver) && (fiter->first.x < riter->second.x) &&
          ...
          (fiter->second.y >= (riter->first.y + overlap)))
      {
        ...
        if (eraseReserved)
        {
          ...
          if (scales) siter = scales->erase(siter);
...
void eraseReservedAreas(const std::string &markerId,
                        ...
                        bool storeCandidates = true)
{
  // If storing candidates, loop the check twice; first get the candidates and remove the reserved
  // areas on second round

  if (storeCandidates)
  {
    for (FillAreas::const_iterator riter = reservedAreas.begin(); (riter != reservedAreas.end());
         riter++)
...
NFmiFillAreas getMarkerArea(const std::string &markerId,
                            const NFmiFillAreas &holeAreas,
                            const FillAreas &reservedAreas,
                            FillAreas &candidateAreas,
                            double mx,
                            double my,
  ...
  do
  {
    ...
    if (!ma.empty())
    {
      ...
      for (it++; (it != ma.end()); it++)
      {
        ...
        freeAreas[markerId].scales.push_back(
            std::make_pair<double, double>(markerWidth / w, markerHeight / h));
      }
    }
    else
    {
...
NFmiFillRect selectMarkerPos(const std::string &markerId,
                             NFmiFillAreas &areas,
                             std::list<std::pair<double, double> > &scales,
                             FillAreas &freeAreas,
                             double x,
                             double y,
                             std::list<std::pair<double, double> >::iterator &sits,
                             bool storeFreeAreas = true)
{
  ...
  {
    double cx = fit->first.x + ((fit->second.x - fit->first.x) / 2),
           cy = fit->first.y + ((fit->second.y - fit->first.y) / 2);
...
void moveMarker(Texts &texts,
                FillAreas freeAreas,
                FillAreas::iterator &rit,
                std::list<std::pair<double, double> >::iterator &sit,
                const NFmiFillRect &r)
{
  // Release old and reserve new position

  // NFmiFillRect rr(rit->second.fillAreas.front());
  // fprintf(stderr,">> %s Release Old %.0f,%.0f -
  // %.0f,%.0f\n",rit->first.c_str(),rr.first.x,rr.first.y,rr.second.x,rr.second.y);
  // fprintf(stderr,">> %s %s Reserve New %.0f,%.0f -
  ...
  texts[rit->first + "SCALE"] << std::fixed << std::setprecision(1) << "scale("
                              << rit->second.scale * sit->first << ","
                              << rit->second.scale * sit->second << ")";
 ...
 *		R3	C3  Marker 3 has reserved area R3 and it's candidate area C3 is reserved by
 *R4
 *and by R5
 *		R2	C2	Marker 2 has reserved area R2 and it's candidate area C2 is reserved
 *by R3
 *			C1	Marker 1 has no free areas and it's candidate area C1 is reserved by
 *R2
 *
 *		==>
                    ...
                    std::list<std::pair<double, double> >::iterator *sit2 = nullptr)
{
  ...
  // cit->first.c_str());
  if (cit == candidateAreas.end()) return false;

  // If the iterators are passed in, candidate has multiple reservations; trying recursively to
  // release/move other overlapping markers
  ...
  for (; (mit != mend); mit++, it++, sit++)
  {
    ...
    if (find(markerChain.begin(), markerChain.end(), *mit) != markerChain.end()) continue;

    FillAreas::iterator fit = freeAreas.find(*mit);

    if (fit != freeAreas.end())
    {
      // Reserve the area of the current candidate temporarily, and check for (and move into
      // candidate container)
      ...
      eraseReservedAreas(
          fit->first, reservedAreas, candidateAreas, fit->second.fillAreas, &(fit->second.scales));
      ...
      if (!(fit->second.fillAreas.empty()))
      {
        ...
        // Release old and reserve new marker position and set/change marker position and scale in
        ...
        moveMarker(texts,
                   ...
                   rit,
                   csit,
                   selectMarkerPos(rit->first,
                                   fit->second.fillAreas,
                                   fit->second.scales,
                                   freeAreas,
                                   rit->second.x,
                                   rit->second.y,
    ...
    if (fit == freeAreas.end()) markerChain.push_back(*mit);

    if ((fit != freeAreas.end()) || arrangeMarkers(texts,
                                                   reservedAreas,
                                                   freeAreas,
                                                   candidateAreas,
                                                   candidateAreas.find(*mit),
                                                   fillAreas,
                                                   scales,
                                                   markerChain,
                                                   RC + 1))
    {
      ...
      // fprintf(stderr,">> [%d] %s NO reserved areas, RET candidate %.0f,%.0f -
...
void scaleMarkers(Texts &texts,
                  ...
                  double yScale)
{
  // Currently simply scaling with current marker's scale instead of searching for scales down from
  // 1 until markers do not overlap,
  // and without moving the scaled markers withing their original bbox

  NFmiFillRect r(std::make_pair(Point(mx - (markerWidth / 2.0), my - (markerHeight / 2.0)),
                                Point(mx + (markerWidth / 2.0), my + (markerHeight / 2.0))));
  int overlap = fillAreaOverlapMax;

  for (FillAreas::const_iterator rrit = reservedAreas.begin(); (rrit != reservedAreas.end());
       rrit++)
    for (NFmiFillAreas::const_iterator rit = rrit->second.fillAreas.begin();
         (rit != rrit->second.fillAreas.end());
         rit++)
    {
      if ((r.first.x < rit->second.x) && (r.second.x >= (rit->first.x + overlap)) &&
          (r.first.y < rit->second.y) && (r.second.y >= (rit->first.y + overlap)))
      {
        texts[rrit->first + "SCALE"].str("");
        texts[rrit->first + "SCALE"] << std::fixed << std::setprecision(1) << "scale("
                                     << rrit->second.scale * xScale << ","
                                     << rrit->second.scale * yScale << ")";
      }
    }
}
...
void adjustMarkerPosition(double minx,
                          double axisWidth,
                          double xStep,
                          double sOffset,
                          double eOffset,
                          ...
                          double &my)
{
  ...
  //
  // For single elevation areas make adjustment only for document's first and last time instant's
  // elevations.
  // Take the extension offsets (how much the elevation exceeds the begin or end of the x -axis)
  // into
...
void getAreaMarkerPos(Texts &texts,
                      ...
                      const std::string &areaPlaceHolder = "")
{
  ...
  // possible fill areas outside
  // (data may contain an extra time step before and after) the document time range

  NFmiFillMap fillMap;
  NFmiFillAreas fillAreas;
  std::list<DirectPosition>::const_iterator cpend = curvePoints.end(), itcp = curvePoints.begin(),
                                            pitcp = curvePoints.begin();
  std::list<std::string> markerChain;
  ...
  if (isHole)
  {
    // Reserve the area of the hole ('fillAreas' contain fill areas suitable for the label,
    // load/save the area of the hole as fill areas too)
    //
  ...
  for (int npx = 0, nX = 0, n0 = 0; (npx < ((int)_lopx.size()));)
  {
    ...
    if (lopx > 0)
    {
      ...
      if (bbCenterMarker && (npx > 0))
      {
        ...
        if ((nX == 0) || (lopx > _lopx[0])) _lopx[0] = lopx;
        if ((nX == 0) || (hipx < _hipx[0])) _hipx[0] = hipx;
      }
    ...
    if (((lopx > 0) && (npx < ((int)_lopx.size()))) || (nX == 0)) continue;
    ...
    if (!bbCenterMarker)
    {
      ...
      if (area != fillAreas.end())
      {
             ...
             area++)
        {
          ...
          // <= (axisHeight + ((area->second.y - area->first.y) / 3)) ? "Pick" : "Skip",n0,n0 +
          // timeSteps,(minx + ((n0 + timeSteps) *
          // xStep)),area->first.x,area->first.y,area->second.x,area->second.y);
        }
        ...
        if (!areas.empty())
        {
          ...
          if (!areas.empty())
          {
            if (areasOut && (!areaPlaceHolder.empty()))
            {
              ...
              {
                texts[areaPlaceHolder]
                    << "<rect x=\"" << iter->first.x << "\" y=\"" << iter->first.y << "\" width=\""
                    << iter->second.x - iter->first.x << "\" height=\""
                    << iter->second.y - iter->first.y << "\" fill=\"" << clrs[clrIdx % clrCnt]
            ...
            ////fprintf(stderr,"\tFR Reserve bl=%.0f,%.0f
      ...
      bool nearest = false;  // Set if the primary position (middle elevation) did not have enough
                             // room for the marker
      bool multiple = false,
           fvmul = false;  // Set if using center of 2 middle elevations as marker position
      ...
      //			int selfvoff = -1;					// Index
      // of favoured free marker position

      for (; (n2 <= nN);)
      {
        if (nearest ||
            (nX % 2) /* The holes are now checked with fill areas: || hasHole[n1] || hasHole[n2] */)
        {
          if (nearest || ((nX % 2) == 0))
          {
        ...
        // Check the current x -position and 1/4 and 1/2 timesteps backwards/forwards if there's
        // free space for the marker.
        //
        ...
        NFmiFillAreas markerArea = getMarkerArea(mId,
                                                 holeAreas,
                                                 reservedAreas,
                                                 freeAreas,
                                                 candidateAreas,
        ...
        if (!markerArea.empty())
        {
          ...
          //
          areas.push_back(std::make_pair(Point(x - (markerWidth / 2), my - (markerHeight / 2)),
                                         Point(x + (markerWidth / 2), my + (markerHeight / 2))));
      ...
      if ((areas.size() > 0) || arrangeMarkers(texts,
                                               reservedAreas,
                                               freeAreas,
                                               candidateAreas,
                                               candidateAreas.find(mId),
                                               areas,
                                               scales,
                                               markerChain))
      {
      ...
      // the heighest free (selfrh) fill area
      //				//
      //				NFmiFillAreas::const_iterator it = areas.begin();
      //				std::list<std::pair<double,double> >::const_iterator sit =
      // scale.begin();
      //
      //				if (selhoff < 0)
      ...
      else
      {
        ...
        // Scale marker down to minimum size and adjust x -position if using areas first or last
        // elevation
    ...
    // Note: Symbols are centered to the selected position, labels have their height as the rendered
    // y -coordinate (x is 0);
    //		 adjust the position (used as tranformation offset) for a label to center it to the
    // selected position
    ...
    reservedAreas[mId].fillAreas.push_back(
        std::make_pair(Point(mx - (markerWidth / 2.0), my - (markerHeight / 2.0)),
                       Point(mx + (markerWidth / 2.0), my + (markerHeight / 2.0))));
    reservedAreas[mId].scale = markerScale;
    reservedAreas[mId].scales.push_back(std::make_pair(xScale, yScale));
    // fprintf(stderr,"\tNF Reserve bl=%.0f,%.0f tr=%.0f,%.0f\n",mx - (markerWidth / 2.0),my -
    // (markerHeight / 2.0),mx + (markerWidth / 2.0),my + (markerHeight / 2.0));
...
void SvgRenderer::renderAreaLabels(const std::list<DirectPosition> &curvePoints,
                                   NFmiFillAreas &holeAreas,
                                   const std::string &confPath,
                                   const std::string &textName,
                                   ...
                                   const std::string &areaPlaceHolder)
{
  ...
  if (!label.empty())
  {
    // Render the label to fixed position (0,0). The final positions are set afterwards by using
  ...
  getAreaMarkerPos(texts,
                   ...
                   minX,
                   axisManager->axisHeight(),
                   axisManager->axisWidth(),
                   axisManager->xStep(),
                   minLabelPosHeight,
                   sOffset,
                   eOffset,
  ...
  std::string svg = texts[textName].str(), textPosId0("--" + textPosId + "--"),
              scaleId0("--" + textPosId + "SCALE--");
  std::list<double>::const_iterator itX, itY, sitX, sitY;

  int placeHolderIndex = 1;

  // fprintf(stderr,"\nSvg: %s\n",svg.c_str());
  for (itX = labelX.begin(), itY = labelY.begin(), sitX = scaleX.begin(), sitY = scaleY.begin();
       (itX != labelX.end());
...
template <typename T>
void SvgRenderer::renderAreaSymbols(const T &cg,
                                    const std::list<DirectPosition> &curvePoints,
                                    NFmiFillAreas &holeAreas,
                                    const std::string &confPath,
                                    ...
                                    bool &aboveTop)
{
  ...
  if (cg.globalScope()) scope.push_back(cg.globalScope());
  ...
  if (!isSet) height = defaultSymbolHeight;

  int width = configValue<int>(scope, confPath, "width", s_optional, &isSet);
  if (!isSet) width = defaultSymbolWidth;

  double scale = configValue<double>(scope, confPath, "scale", s_optional, &isSet);
  if (!isSet) scale = 1.0;

  int minSymbolPosHeight =
      configValue<int>(scope, confPath, "minsymbolposheight", s_optional, &isSet);
  if ((!isSet) || (minSymbolPosHeight < (height * symbolPosHeightFactorMin)))
  {
    double symbolPosHeightFactor =
        configValue<double>(scope, confPath, "symbolposheightfactor", s_optional, &isSet);
  ...
  for (itx = symbolX.begin(), ity = symbolY.begin(), sitx = scaleX.begin(), sity = scaleY.begin();
       (itx != symbolX.end());
       itx++, ity++, sitx++, sity++, placeHolderIndex++)
  {
    render_aerodromeSymbol(confPath,
...
bool CategoryValueMeasureGroup::groupMember(bool first, const woml::CategoryValueMeasure *cvm)
{
  ...
  if (!cvm)
    throw std::runtime_error(
        "CategoryValueMeasureGroup::groupMember: CategoryValueMeasure value expected");
...
void addCurvePosition(std::vector<DirectPosition> &curvePositions,
                      ...
                      bool leftOnly = false)
{
  // Add curve point and if x- (and additionally y-) offset is given, add points on both sides too
  // (to control the curve shape on corners)
  // if leftOnly is false; if leftOnly is true, add only the left side point (going up to the left
  // side hi range point
  // when closing the path to the starting top left corner).
  //
  // Note: y -offset is passed in for top left ('fst') and bottom right ('edn' followed by 'bwd')
  // corners; for top right ('fwd' + 'edn') and
  //		 bottom left ('bwd' + 'eup') corner smoothenCurve() is used

  if (fabs(xOffset) >= 1.0)
  {
    addCurvePosition(curvePositions,
                     curvePositions0,
                     DirectPosition(x - xOffset, y + yOffset),
                     DirectPosition(x - xOffset, y0 + yOffset));
  ...
  addCurvePosition(curvePositions,
                   curvePositions0,
                   DirectPosition(x, y + (yOffset / 2)),
                   DirectPosition(x, y0 + (yOffset / 2)));

  if (fabs(xOffset) >= 1.0)
    addCurvePosition(curvePositions,
                     curvePositions0,
                     DirectPosition(x + xOffset, y),
...
{
  // Smoothen curve corner by adjusting y -coordinates with given offset. The corner has three
  // points (two additional points around the actual
  // elevation lo/hi range point at x,y), spaced by xOffset in x -direction and having originally
  // the same y -coodinate.

  if ((fabs(xOffset) < 1.0) || (fabs(yOffset) < 1.0) || (curvePositions.size() < 3)) return;

  if (fabs(yOffset) > (height / 10)) yOffset = ((yOffset < 0) ? -1 : 1) * (height / 10);
                                       ...
                                       bool nonGndVdn2Gnd)
{
  ...
  // can't exceed
  // half of the timestep width (an intermediate point is always generated halfway of subsequent
  // elevations)

  xOffset = ((xOffset < 0.0) ? 0.0 : std::min(xOffset, (xStep / 2)));

  curvePositions.clear();
  curvePositions0.clear();
  ...
  for (iteg = egbeg, phase = pphase = fst; (iteg != egend);)
  {
    ...
    if ((lo > 0.0) && (lo < nonZ)) lo = 0.0;

    bool ground = (checkGround && (lo < nonZ));

    if (isVisible && (hi >= nonZ)) *isVisible = true;
...
//
      ...
      {
        if (ground && (vt > bt))
          path << "M" << (x - xStep) << "," << axisManager->axisHeight() << " L" << x << ","
               << hipx;
        else
          path << "M" << x << "," << hipx;
      }
...
//
      ...
      if (options.debug) path << " L" << x << "," << hipx;
    }
    else if (phase == vup)
    {
...
//
    ...
    {
// Move up connecting "nonground" (or non ZeroTolorence) elevation's lo range point to hi range
// point
//
// Note: ZeroTolorence "ground" elevation's hi range point can be connected to 0 at previous/next
// time instant
//
#ifdef STEPS
      ...
      if ((!ground) || (vt == bt) || (!(iteg->topConnected())))
      {
        ...
        addCurvePosition(curvePositions,
                         ...
                         lopx0 - ((lopx0 - hipx0) / 2));
        addCurvePosition(
            curvePositions, curvePositions0, x, hipx, hipx0, xOffset, 0.0, iteg->topConnected());

        if (options.debug) path << (((vt != bt) && (vt != et)) ? " L" : " M") << x << "," << hipx;
      }
...
//
// If time instant of the last "ground" (zeroTolerance) elevation in the group is not forecast's
// last
...
//
      ...
      if (options.debug)
      {
        if (ground && (vt < et)) path << " L" << (x + xStep) << "," << axisManager->axisHeight();
...
//
      ...
      if (smoothen)
      {
        ...
        if (&curvePositions != &curvePositions0)
          smoothenCurve(
              curvePositions0, x + xStep, plopx0, plopx0 - phipx0, -xOffset, -yOffset, true);
      }
    ...
    {  // lst
      if (single)
      {
        ...
        //
        // Note: Using the center of the top edge as starting point to get symmetrical (ends to the)
        // result curve
        //
        ...
        if (options.debug)
        {
          ...
          path << "M" << leftSide << "," << hipx << " L" << rightSide << "," << hipx << " L"
               << rightSide << "," << lopx << " L" << leftSide << "," << lopx << " L" << leftSide
               << "," << hipx;
        }
        ...
        addCurvePosition(curvePositions,
                         curvePositions0,
                         rightSide + ((vSSOffset < 0.0) ? 0.0 : vSSOffset),
                         lopx - ((lopx - hipx) / 2),
                         lopx0 - ((lopx0 - hipx0) / 2));
        ...
        addCurvePosition(curvePositions,
                         curvePositions0,
                         leftSide - ((vSSOffset < 0.0) ? 0.0 : vSSOffset),
                         lopx - ((lopx - hipx) / 2),
                         lopx0 - ((lopx0 - hipx0) / 2));
  ...
  if (!single)
  {
    // Use the second last curve position - in the middle of the left side of the starting / top
    // left elevation - as
    // the starting position to eliminate the "starting n�kk�nen" effect
    //
...
{
  // 'cloudSet' stores cloud types (their starting positions in the group's 'types' setting)
  // contained in current group;
  ...
  cloudLayerConfig(confPath,
                   tightness,
                   borderCompensation,
                   minLabelPosHeight,
                   labelPosHeightFactor,
                   cloudGroupCategory.groups(),
                   cloudSet);
  ...
  for (;;)
  {
    ...
    //
    // Note: Holes are loaded first; they (fill areas extracted from them) are added to reserved
    // areas
    ...
    //
    // Use cloud decoration offsets and get the unmodified curve baseline too (for label
    // positioning)
    ...
    scaledCurvePositions(eGrp,
                         bt,
                         et,
                         curvePositions,
                         borderCompensation ? curvePositions0 : curvePositions,
                         scaledLo,
                         scaledHi,
                         hasHole,
                         itcg->xOffset(),
    ...
    if (options.debug)
      texts[itcg->placeHolder() + "PATH"] << "<path class=\"" << itcg->classDef() << "\" id=\""
                                          << "CloudLayersB" << nGroups << "\" d=\"" << path.str()
                                          << "\"/>\n";
    ...
    // itcp->getX() << "," << itcp->getY();
    // pnts << "<circle cx=\"" << itcp->getX() << "\" cy=\"" << itcp->getY()+150 << "\" r=\"5\"
    // stroke=\"black\" stroke-width=\"1\" fill=\"blue\"/>"; } std::cerr << std::endl << "<<< PNT";
    // }
    ...
    //{ std::cerr << std::fixed << std::setprecision(1);
    // std::list<DirectPosition>::iterator cpbeg = curvePoints.begin(),cpend =
    // curvePoints.end(),itcp; size_t n = 0;
    // for (itcp = cpbeg; (itcp != cpend); itcp++, n++) {
    // if ((n % 10) == 0) std::cerr << std::endl << n << ": "; else std::cerr << " "; std::cerr <<
    // itcp->getX() << "," << itcp->getY()+150; } std::cerr << std::endl << "<<< STP" << std::endl;
    // }
    bm.decorateCurve(curvePoints,
                     isHole,
                     itcg->scaleHeightMin(),
                     itcg->scaleHeightRandom(),
                     itcg->controlMin(),
                     itcg->controlRandom(),
                     decoratorPoints);
    ...
    std::list<DirectPosition>::const_iterator cpbeg = curvePoints.begin(),
                                              cpend = curvePoints.end(), itcp;
    ...
    if (!label.empty())
    {
      // For a hole store it's area as reserved, otherwise render the label to each separate visible
      // part of the cloud
      //
...
bool joinElevations(ElevGrp &eGrp)
{
  ...
  for (piteg = iteg; (iteg != egend);)
  {
    ...
    if ((dh == 0) && (plo < hi))
    {
      // Elevations overlap vertically; store the joined range to the upper and erase the lower
      // elevation
      //
      lo = std::min(plo, lo);
      hi = phi;
    ...
    CategoryValueMeasureGroup *categoryGroup)
{
  ...
  {
    // Load the time instants within the document's time range plus one time instant before the
    // starting and after the
    // ending time instant (if available), thus extending the document's time range (by 1 hour) in
    // both directions.
    //
    // Extendind the time range gives some hints how the clouds, icing etc. continue outside the
    // visible time window.
    //
    // All data is rendered, but only the document's original time range is assumed to be visible;
    ...
    if (vt < bt)
    {
      ...
      if ((nitts == tsend) || (nitts->validTime() != bt) || ((bt - vt).hours() != 1)) continue;
    ...
    else
    {
      if ((!all) && (eGrp.size() > 0))
      {
        ...
        //
        if ((vt - pitts->validTime()).hours() > 1) break;
      }
    ...
    {
      std::list<woml::GeophysicalParameterValue>::iterator itpv =
          itpvs->get()->editableValues().begin();
      std::list<woml::GeophysicalParameterValue>::iterator pvend =
          itpvs->get()->editableValues().end();

      for (; (itpv != pvend); itpv++)
      {
        // Not mixing holes and nonholes
        //
        if (((eGrp.size() > 0) || getHole) &&
        ...
        if (!all)
        {
          // Check if group number and value category (cloud type or magnitude for icing or
          // turbulence) matches.
          //
          const woml::CategoryValueMeasure *cvm =
              dynamic_cast<const woml::CategoryValueMeasure *>(itpv->value());
          ...
          if (!(group->groupMember((eGrp.size() == 0), cvm)))
          {
            if ((eGrp.size() == 0) && options.debug)
              debugoutput << "Settings for " << cvm->category() << " not found\n";
        ...
        if (byCategory && categoryGroup->standalone())
          // Note: Vertical overlapping is currently not checked for standalone group category;
          // should load
          //		 all elevations for the time instant, join and return the first elevation if
          // required
          //
  ...
  //
  // Note: If nonground and ground elevations are joined, the group type returned might be logically
  // wrong
  //		 ('mixed' instead of 'ground'); currently this is not an issue.

  if (join) joinElevations(eGrp);
...
void SvgRenderer::checkLeftSide(ElevGrp &eGrp,
                                ElevationHole &hole,
                                CategoryValueMeasureGroup *groupCategory)
{
  ...
  for (; (liteg != egend); liteg++)
  {
    ...
    if (dh == -1)
    {
      if (groupCategory)
      {
        // Check that elevation belongs to the same category (cloud type or icing/turbulence
        // magnitude) as the hole
        //
        ElevGrp::iterator iteg = liteg.base();
        iteg--;

        if (!checkCategory(groupCategory, hole.aboveElev, iteg)) continue;
      }
      ...
      if (hi < hole.hi) continue;

      if ((lo > 0.0) && (lo < axisManager->nonZeroElevation())) lo = 0.0;
...
bool SvgRenderer::checkLeftSideHoles(ElevationHoles &holes,
                                     ...
                                     CategoryValueMeasureGroup *groupCategory)
{
  ...
  for (; (liteh != ehend); liteh++)
  {
    ...
    if (dh == -1)
    {
      if (groupCategory)
      {
        // Check that the holes belong to the same category (cloud type or icing/turbulence
        // magnitude)
        //
        if (!checkCategory(groupCategory, iteh->aboveElev, liteh->aboveElev)) continue;
      }
...
void SvgRenderer::checkRightSide(ElevGrp &eGrp,
                                 ElevationHoles::iterator &iteh,
                                 CategoryValueMeasureGroup *groupCategory)
{
  ...
  for (riteg++; (riteg != egend); riteg++)
  {
    ...
    if (dh == 1)
    {
      if (groupCategory)
      {
        ...
        // magnitude) as the hole
        //
        if (!checkCategory(groupCategory, iteh->aboveElev, riteg)) continue;
      }
...
bool SvgRenderer::checkRightSideHoles(ElevationHoles &holes,
  ...
  for (; (riteh != ehend); riteh++)
  {
    ...
    if (dh == 1)
    {
      if (groupCategory)
      {
        ...
        // magnitude)
        //
        if (!checkCategory(groupCategory, iteh->aboveElev, riteh->aboveElev)) continue;
      }
...
bool SvgRenderer::checkBothSideHoles(ElevationHoles &holes,
                                     ElevationHoles::iterator &iteh,
                                     CategoryValueMeasureGroup *groupCategory)
{
...
bool SvgRenderer::checkHoles(ElevGrp &eGrp,
                             ElevationHoles &holes,
                             CategoryValueMeasureGroup *groupCategory,
                             bool setNegative)
{
  // Check if the holes are closed by left side closed holes on the left side or by elevations on
  // the right side.
  ...
  for (; (iteh != ehend); iteh++)
  {
    if ((!(iteh->leftClosed)) && iteh->leftAboveBounded &&
        checkLeftSideHoles(holes, iteh, groupCategory))
      iteh->leftClosed = true;

    if (iteh->leftClosed) checkRightSide(eGrp, iteh, groupCategory);
  }
  ...
  for (; (riteh != rehend); riteh++)
    if (riteh->leftClosed && (!(riteh->rightClosed)) && riteh->rightAboveBounded &&
        checkRightSideHoles(holes, riteh, groupCategory))
      riteh->rightClosed = true;

  // Check if all overlapping holes have both sides closed. Repeat until no "leaks" are found (all
  // overlapping holes
  // are set to open if needed), rolling the status changes (set to open) to neighbour holes one
  // hole at a time.
  ...
  do
  {
    ...
    for (iteh = holes.begin(); (iteh != ehend); iteh++)
      if ((iteh->leftClosed != iteh->rightClosed) ||
          (iteh->leftClosed && (!checkBothSideHoles(holes, iteh, groupCategory))))
      {
  ...
  for (iteh = holes.begin(); (iteh != ehend); iteh++)
  {
    ...
    if (setNegative) holeElev->isNegative(true);
  }
...
void generateHoleElevation(ElevGrp &eGrp, ElevGrp::iterator &piteg, ElevGrp::iterator &iteg)
{
  ...
  const woml::CategoryValueMeasure *cvmA =
      dynamic_cast<const woml::CategoryValueMeasure *>(piteg->Pv()->value());

  if (!cvmA) throw std::runtime_error("generateHoleElevation: CategoryValueMeasure value expected");
  ...
  std::list<woml::GeophysicalParameterValue>::iterator itpv =
      piteg->Pvs()->get()->editableValues().end();
  itpv--;

  // Add new object to the elevation group. Flag it as a hole (already) to enable simple detection
  // when finished
...
bool SvgRenderer::checkCategory(CategoryValueMeasureGroup *groupCategory,
                                ElevGrp::iterator &e) const
{
  const woml::CategoryValueMeasure *cvm =
      dynamic_cast<const woml::CategoryValueMeasure *>(e->Pv()->value());

  if (!(groupCategory->groupMember(cvm)))
  {
    if (options.debug) debugoutput << "Settings for " << cvm->category() << " not found\n";
...
{
  const woml::CategoryValueMeasure *cvm1 =
      dynamic_cast<const woml::CategoryValueMeasure *>(e1->Pv()->value());
  const woml::CategoryValueMeasure *cvm2 =
      dynamic_cast<const woml::CategoryValueMeasure *>(e2->Pv()->value());

  if (!(groupCategory->groupMember(true, cvm1)))
  {
    if (options.debug) debugoutput << "Settings for " << cvm1->category() << " not found\n";
...
bool SvgRenderer::searchHoles(const std::list<woml::TimeSeriesSlot> &ts,
                              CategoryValueMeasureGroup *groupCategory,
                              bool setNegative)
{
  // Get all elevations.
  //
  // Note: 'join' controls whether vertically overlapping elevations are joined or not. It must be
  // passed in as false here
  //		 because some features have additional grouping factor (cloud type for CloudLayers
  // and
  // magnitude for Icing and
  //		 Turbulence), and joining can only be made after each overlapping subgroup of
  // elevations
  // having matching/configured
  ...
  if (groupCategory)
  {
    // For categorized data the elevations are scanned one category (cloud type or icing/turbulence
    // magnitude) at a time.
    // Elevations with unknown category are first removed from the underlying woml object
    // collection.
    //
    bool reLoad = false;
  ...
  do
  {
    for (reScan = false, iteg = egbeg, piteg = egend; (iteg != egend);
         nextItem(iteg, scanned, reScan))
    {
      // No need to check the first and last time instants (holes would be open on the left or right
      // side).
      //
      ...
      if (piteg != egend)
      {
        ...
        {
          // Generate a new elevation for the previous time instant to store the hole touching
          // ground.
          //
        ...
        {
          // Check first that above and below elevation belong to the same category (cloud type or
          // icing/turbulence magnitude).
          //
          if ((!groupCategory) || checkCategory(groupCategory, piteg, iteg))
          {
            // For overlapping elevations only the first (having max hi value) is taken into
            // account.
            //
            if (hi >= plo) continue;

            // Check if the hole is closed or bounded by an elevation on the left side. Store the
            // hole.
            //
            ElevationHole hole;
  ...
  if (holes.size() > 1) holes.sort();

  // Check which of the holes are closed (if any). For closed holes the bounding elevations are
  // modified
  // in the underlying woml object collection. For nonclosed holes the generated elevations (if any)
  // are
  // deleted.
...
void SvgRenderer::render_timeserie(const woml::Contrails &contrails)
{
  ...
  try
  {
    ...
    for (;;)
    {
      ...
      if (eGrp.size() == 0) break;

      // Loop thru the elevations connecting first the continuous visible lo range points and then
      // the
      ...
      for (int rng = 0; (rng < 2); rng++)
      {
        ...
        for (iteg = prevIteg = eGrp.begin(); (iteg != egend);)
        {
          ...
          // scaledElevation() returns negative value and 'above' is set if the scaled value exceeds
          // y -axis height.
          ...
          bool prevMissing = ((val > 0) && (x > (xStep / 2)) &&
                              ((x > (prevX + xStep + (xStep / 2))) || (prevVal <= 0)));
          ...
          if (val > 0)
          {
            ...
            if (prevMissing)
            {
              ...
              if (prevAbove)
                ...
                //
                path << " L" << (x - (xStep / 2)) << "," << axisManager->scaledElevation(hi) << " M"
                     << (x - (xStep / 2)) << "," << val;
            ...
            // Because the data can now exceed the visible x -axis, checking/setting visibility here
            // by checking the x -coordinate too

            if ((x >= 0) && (x < (axisWidth + 1)))
            {
              visible = true;

              if (rng == 1) hiVisible = true;
            }
          }
          else
          {
            ...
            //
            if (above && (x >= 0) && (x < (axisWidth + 1))) aboveTop = true;

            if (pntCnt > 0) segmentDrawn = true;
          ...
          if ((iteg == egend) && ((pntCnt > 0) || segmentDrawn))
          {
            ...
            texts[CONTRAILS] << "<path class=\"" << classDef << "Visible"
                             << "\" id=\"" << contrailId << nGroups << "\" d=\"" << path.str()
                             << "\"/>\n";
          }
        }  // for iteg
      }    // for rng
  ...
  if (options.debug) debugoutput << "Settings for " << confPath << " not found\n";
                                                   ...
                                                   std::set<size_t> &memberSet)
{
  ...
  try
  {
    ...
    // If bbcenterlabel is true (default: false), group's symbol is positioned to the center of the
    // area's bounding box;
    ...
    for (int i = 0, globalsIdx = -1; i < groupSpecs.getLength(); i++)
    {
      ...
      bool symbolOnly =
          configValue<bool>(group, confPath, "symbolonly", globalScope, s_optional, &isSet);
      if (!isSet) symbolOnly = false;
      ...
      double xOffset =
          configValue<double>(group, confPath, "xoffset", globalScope, s_optional, &isSet);
      if (!isSet)
        xOffset = 0.0;
      else
        xOffset *= xStep;

      double yOffset = (double)configValue<int, double>(
          group, confPath, "yoffset", globalScope, s_optional, &isSet);
      if (!isSet) yOffset = 0.0;
      ...
      double vOffset =
          configValue<double>(group, confPath, "voffset", globalScope, s_optional, &isSet);
      if (!isSet)
        vOffset = 0.0;
      else
        vOffset *= xStep;
      ...
      double vSOffset =
          configValue<double>(group, confPath, "vsoffset", globalScope, s_optional, &isSet);
      if (!isSet)
        vSOffset = xStep / 3;
      else
        vSOffset *= xStep;
      ...
      double vSSOffset =
          configValue<double>(group, confPath, "vssoffset", globalScope, s_optional, &isSet);
      if (!isSet)
        vSSOffset = vSOffset;
      else
        vSSOffset *= xStep;
      ...
      std::string placeHolder(boost::algorithm::trim_copy(
          configValue<std::string>(group, confPath, "output", globalScope, s_optional)));
      if (placeHolder.empty()) placeHolder = FEATURE;

      std::string labelPlaceHolder(boost::algorithm::trim_copy(
          configValue<std::string>(group, confPath, "labeloutput", globalScope, s_optional)));
      if (labelPlaceHolder.empty()) labelPlaceHolder = placeHolder + "TEXT";

      // Standalone types (only one type in the group) have flag controlling whether the group is
      // combined or not

      bool combined =
          configValue<bool>(group, confPath, "combined", globalScope, s_optional, &isSet);
      if (!isSet) combined = true;
                                   ...
                                   const std::string &classNameExt)
{
  ...
  //
  // Member set contains the types (icing or turbulence magnitudes; their starting positions in the
  // group's 'types' setting)
  // in current group; if neither group's symbol nor label is given, label (comma separated list of
  // types) is build using the order
  // the types are defined in 'types' setting.
  //
  // The member set is cleared and then set by all the groups; types contained are added to the set
  // when collecting a group
  // in elevationGroup().
  ...
  groupConfig<GroupTypeT>(confPath,
                          tightness,
                          minLabelPosHeight,
                          labelPosHeightFactor,
  ...
  for (;;)
  {
    ...
    bool single = scaledCurvePositions(eGrp,
                                       bt,
                                       et,
                                       curvePositions,
                                       curvePositions,
    ...
    if (!asSymbol)
    {
      ...
      std::list<DirectPosition>::iterator cpbeg = curvePoints.begin(), cpend = curvePoints.end(),
                                          itcp;
      ...
      ///<< "\"/>";
      // pnts << "<circle cx=\"" << itcp->getX() << "\" cy=\"" << itcp->getY() << "\" r=\"2\"
      // stroke=\"black\" stroke-width=\"1\" fill=\"red\"/>"; }
      // texts[FEATURE] << pnts.str();
      //}
    }

    // For a hole store it's area as reserved, otherwise render the single symbol or one symbol or
    // label
    ...
    if ((!isHole) && (!symbol.empty()))
      ...
      renderAreaLabels(curvePoints,
                       ...
                       "ZZ" + FEATURE + "Text",
                       isHole,
                       (symbol.empty() && (!isHole)) ? groupCategory.groupLabel() : "",
                       itcg->bbCenterMarker(),
                       minX,
                       minLabelPosHeight,
                       labelPosHeightFactor,
...
{
  render_timeserie<IcingGroupCategory, IcingGroup>(
      icing.timeseries(), "Icing", icing.classNameExt());
}
...
{
  render_timeserie<TurbulenceGroupCategory, TurbulenceGroup>(
      turbulence.timeseries(), "Turbulence", turbulence.classNameExt());
}
...
{
  if (windDirection < 0.0) windDirection = fabs(windDirection);
...
WindArrowOffsets calculateWindArrowOffsets(unsigned int arrowArmLength, double windDirection)
{
  ...
  if (arrowArmLength == 0) return windArrowOffsets;

  if ((fabs(windDirection - 90.0) > 0.001) || (fabs(windDirection - 270.0) > 0.001))
    windArrowOffsets.verticalOffsetPx =
        (int)floor(((arrowArmLength / 2.0) * sin(windDirectionToRadians(windDirection))) + 0.5);

  if ((fabs(windDirection) > 0.001) || (fabs(windDirection - 180.0) > 0.001))
    windArrowOffsets.horizontalOffsetPx =
        (int)floor(((arrowArmLength / 2.0) * cos(windDirectionToRadians(windDirection))) + 0.5);
...
void SvgRenderer::render_timeserie(const woml::Winds &winds)
{
  ...
  try
  {
    ...
    // time
    // instant (-1-, the default), by using the x -coordinate of document's first time instant if
    // there
    // are winds for one time instant only (-2-) or by using the first wind time instant as is
    // (-3-).
    //
    ...
    for (itts = tsbeg; (itts != tsend); itts++, nSymbols = 0)
    {
      ...
      for (itpvs = itts->values().begin(); (itpvs != pvsend); itpvs++)
      {
        ...
        //
        std::list<woml::GeophysicalParameterValue>::const_iterator pvend =
                                                                       itpvs->get()->values().end(),
                                                                   itpv;
        ...
        for (itpv = itpvs->get()->values().begin(); (itpv != pvend); itpv++)
        {
          ...
          const woml::FlowDirectionMeasure *fdm =
              dynamic_cast<const woml::FlowDirectionMeasure *>(itpv->value());

          if (!fdm) throw std::runtime_error(valtypeMsg);
          ...
          if (cols.size() != 2)
            ...
            {
              throw std::runtime_error(std::string(directionMsg) + fdm->directionVector() +
                                       "': " + ex.what());
            }

          const libconfig::Setting *condSpecs =
              matchingCondition(config, confPath, confPath, "", -1, ws);
          ...
          if (condSpecs)
          {
            ...
            bool isSet;
            double scale =
                configValue<double, int>(*condSpecs, confPath, "scale", &specs, s_optional, &isSet);
            if (!isSet) scale = 1.0;

            // Wind arrow arm length (in pixels) to calculate horizontal and vertical offsets to
            // position the arrowheads to
            // the elevation lines. Without adjustment the wind arrows are (assumed to be) centered
            // to the elevation lines.

            unsigned int arrowArmLength = configValue<unsigned int>(
                *condSpecs, confPath, "arrowarmlength", &specs, s_optional, &isSet);
            if (!isSet) arrowArmLength = 0;
            ...
            if (fstts == tsend) fstts = itts;

            if (itts == fstts)
            {
              // In 'autoWindBase' mode x -coordinates of the first time instant will be set
              // afterwards
              //
              ...
              if ((!useWindBase) || autoWindBase || (arrowArmLength > 0))
              {
                ...
                if (autoWindBase) windArrowOffsetsList.push_back(windArrowOffsets);
              }
              ...
              // Wind symbol
              //
              texts[WINDBASE] << "<g transform=\"translate(" << windXPos << "," << std::fixed
                              << std::setprecision(1) << y << ") scale(" << std::fixed
                              << std::setprecision(1) << scale << ")\" class=\"" << classDef
                              << "\">\n"
                              << "<use transform=\"rotate(" << std::fixed << std::setprecision(1)
                              << wd << ")\" xlink:href=\"" << svgescape(uri + "#" + symbol)
                              << "\"/>\n</g>\n";
            ...
            else
            {
              ...
              if (bgXPos > 0.0)
              {
                ...
                if ((nSymbols == 0) && (!href.empty()))
                  texts[WINDEXTRA] << "<use transform=\"translate(" << std::fixed
                                   << std::setprecision(1) << bgXPos << ",0)\" xlink:href=\""
                                   << href << "\"/>\n";

                texts[WINDEXTRA] << "<g transform=\"translate(" << std::fixed
                                 << std::setprecision(1) << windXPos << "," << std::fixed
                                 << std::setprecision(1) << y << ") scale(" << std::fixed
                                 << std::setprecision(1) << scale << ")\" class=\"" << classDef
                                 << "\">\n"
                                 << "<use transform=\"rotate(" << std::fixed << std::setprecision(1)
    ...
    if ((fstts != tsend) && useWindBase)
    {
      if ((!autoWindBase) || singleTime)
      {
        ...
        if (!windBaseHour.empty())
          texts[WINDBASEHOUR] << boost::algorithm::replace_first_copy(
              windBaseHour,
              "%hour%",
              toFormattedString(fstts->validTime(), "HH", axisManager->utc()).CharPtr());
      }
      ...
      if (autoWindBase)
      {
        ...
        //
        std::list<WindArrowOffsets>::iterator ofend = windArrowOffsetsList.end(),
                                              itoff = windArrowOffsetsList.begin();
...
unsigned int SvgRenderer::getLeftSideGroupNumber(ElevGrp &eGrp,
                                                 ...
                                                 bool mixed)
{
  ...
  for (; (liteg != egend); liteg++)
  {
    ...
    if (dh == -1)
    {
      ...
      // 'mixed' controls whether or not ground and nonground elevations are included into same
      // group.
      ...
      if ((leftGroupNumber > 0) && (leftGroupNumber < groupNumber)) groupNumber = leftGroupNumber;
    }
    else if (dh != 0)
      break;
  }
...
unsigned int SvgRenderer::getRightSideGroupNumber(ElevGrp &eGrp,
...
void SvgRenderer::setGroupNumbers(const std::list<woml::TimeSeriesSlot> &ts, bool mixed)
{
  ...
  do
  {
    // For each elevation, set minimum group number from overlapping left and right side elevations
    // (if any).
    ...
    for (; (iteg != egend); iteg++)
    {
      ...
      groupNumber = getLeftSideGroupNumber(
          eGrp, iteg, ((currentGroupNumber == 0) ? nextGroupNumber : currentGroupNumber), mixed);
      ...
      if ((currentGroupNumber == 0) || (groupNumber < currentGroupNumber))
      {
        ...
        if (groupNumber == nextGroupNumber) nextGroupNumber++;
      }
    }
    ...
    {
      groupNumber =
          getRightSideGroupNumber(eGrp, riteg, currentGroupNumber = riteg->groupNumber(), mixed);
...
void SvgRenderer::render_timeserie(const woml::ZeroTolerance &zerotolerance)
{
  ...
  try
  {
    ...
    // If empty label is given, no label.
    //
    // If bbcenterlabel is true (default: false), the label is positioned to the center of the
    // area's bounding box;
    // otherwise using 1 or 2 elevations in the middle of the area in x -direction
    ...
    for (;;)
    {
      ...
      // If a time instant does not have elevation with lo -range <= (or near) 0, below 0 elevation
      // is generated
      ...
      if (options.debug)
        texts[ZEROTOLERANCE + "PATH"] << "<path class=\"" << classDef << "\" id=\""
                                      << "ZeroToleranceB" << nGroups << "\" d=\"" << path.str()
                                      << "\"/>\n";
      ...
      // fprintf(stderr,"> Bez:\n");
      // for (itcp = cpbeg; (itcp != cpend); itcp++) fprintf(stderr,"> Bez x=%.1f,
      // y=%.1f\n",itcp->getX(),itcp->getY());
      for (itcp = cpbeg; (itcp != cpend); itcp++)
      {
        ...
        if ((x >= 0) && (x < (axisWidth + 1)) && (y > 0)) visible = true;
      }

      texts[ZEROTOLERANCE] << "<path class=\"" << classDef << "\" id=\""
                           << "ZeroTolerance" << nGroups << "\" d=\"" << path.str() << "\"/>\n";

      if (visible && setUnvisible)
      {
        // The curve is visible (elevation(s) above zero did exist); hide the fixed template text
        // "Zerolevel is null"
        //
                               ...
                               bool asValue)
{
  ...
  try
  {
    ...
    if (!valSpecs.isList()) throw std::runtime_error(confPath + typeMsg);
      ...
      try
      {
        if (lookup<std::string>(specs, confPath, "name", s_name) == valClass)
        {
          ...
          if (type.empty() || (type == "value"))
          {
            ...
            std::string classDef(configValue<std::string>(
                specs, valClass, "class" + vtype, globalScope, s_optional));
            if (classDef.empty())
              classDef = (vtype.empty() ? valClass : (valClass + " " + valClass + vtype));

            std::string pref(
                configValue<std::string>(specs, valClass, "pref" + vtype, globalScope, s_optional));
            std::string href(
                configValue<std::string>(specs, valClass, "href" + vtype, globalScope, s_optional));

            // Output placeholder; by default output to passed stream

            std::string placeHolder(boost::algorithm::trim_copy(
                configValue<std::string>(specs, valClass, "output", globalScope, s_optional)));

            // Offsets for placing the value (used for wind speed to position the value to the
            // center or to the border
            // of the wind symbol)
            ...
            if (asValue)
            {
              xoffset = static_cast<int>(floor(configValue<float, int>(
                  specs, valClass, "vxoffset", globalScope, s_optional, &isSet)));
              if (!isSet) xoffset = 0;

              yoffset = static_cast<int>(floor(configValue<float, int>(
                  specs, valClass, "vyoffset", globalScope, s_optional, &isSet)));
              if (!isSet) yoffset = 0;
            }
    ...
    if (options.debug)
    {
      ...
      debugoutput << p << valClass << " not found\n";
    }
  }
  catch (libconfig::SettingNotFoundException &ex)
  {
    ...
    //
    if (options.debug)
      debugoutput << "Settings for " << confPath << " (" << valClass << ") not found\n";
  }
}
      ...
      itsElevations()
{
  ...
  try
  {
    ...
    if (!elevAxisSpecs.isGroup()) throw std::runtime_error(confPath + groupMsg);
    ...
    for (int i = 0; i < elevSpecs.getLength(); ++i)
    {
      ...
      if (!specs.isGroup()) throw std::runtime_error(confPath + listMsg);
      ...
      itsElevations.push_back(
          Elevation(elevation, scale, (int)(itsAxisHeight * scale), lLabel, rLabel, line));
    }
                                    ...
                                    double aboveTop)
{
  ...
  std::list<Elevation>::iterator ith =
      lower_bound(itsElevations.begin(), itsElevations.end(), Elevation(elevation));
  ...
  if (ith->elevation() > elevation)
  {
    ...
    return itsAxisHeight -
           (itsAxisHeight * (itl->scale() + ((elevation - itl->elevation()) * itl->factor())));
  }
...
double AxisManager::xOffset(const boost::posix_time::ptime &validTime) const
{
  ...
  return (((double)(validTime - itsTimePeriod.begin()).total_seconds()) /
          itsTimePeriod.length().total_seconds()) *
         itsAxisWidth;
}
...
{
  if (isDeleted()) throw std::runtime_error("Internal error: Pv() called for a deleted elevation");
...
void ElevationGroupItem::isScanned(bool scanned)
{
  ...
  if (scanned) Pv()->setFlags(b_scanned);
}
...
void ElevationGroupItem::isGroundConnected(bool connected)
{
  ...
  if (connected) Pv()->setFlags(b_groundConnected);
}
...
void ElevationGroupItem::isNegative(bool negative)
{
  ...
  if (negative) Pv()->setFlags(b_negative);
}
...
void ElevationGroupItem::hasHole(bool hole)
{
  ...
  if (hole) Pv()->setFlags(b_hasHole);
}

bool ElevationGroupItem::hasHole() const { return ((Pv()->getFlags() & b_hasHole) ? true : false); }

void ElevationGroupItem::isHole(bool hole)
{
  // We only need to set the bit

  if (hole) Pv()->setFlags(b_isHole);
}

bool ElevationGroupItem::isHole() const { return ((Pv()->getFlags() & b_isHole) ? true : false); }

void ElevationGroupItem::groupNumber(unsigned int group)
{
  woml::CategoryValueMeasure *cvm =
      dynamic_cast<woml::CategoryValueMeasure *>(Pv()->editableValue());

  if (!cvm) throw std::runtime_error("groupNumber: CategoryValueMeasure value expected");
...
{
  const woml::CategoryValueMeasure *cvm =
      dynamic_cast<const woml::CategoryValueMeasure *>(Pv()->value());
...
void ConfigGroup::addType(const std::string &theMemberType) const
{
  if (!itsStandalone)
  {
    ...
    if (pos != std::string::npos) itsMemberSet.insert(pos + 1);
  }
}
...
{
  if (itsStandalone) return itsMemberTypes;
...
void SvgRenderer::visit(const woml::CloudArea &theFeature)
{
...
void SvgRenderer::visit(const woml::ColdAdvection &theFeature)
{
...
void SvgRenderer::visit(const woml::ColdFront &theFeature)
{
  ...
  render_front(
      path, paths, coldfronts, id, "coldfront", "coldfrontglyph", Cc, Cc, fontsize, spacing);
}
...
void SvgRenderer::visit(const woml::InfoText &theFeature)
{
  ...
  render_text(texts,
              "Text",
              theFeature.name(),
              (text != options.locale) ? NFmiStringTools::UrlDecode(text) : "",
              x,
              y);
}
...
void SvgRenderer::visit(const woml::JetStream &theFeature)
{
  ...
  render_front(path, paths, jets, id, "jet", "", "", "", fontsize, spacing);
}
...
void SvgRenderer::visit(const woml::MigratoryBirds &theFeature)
{
...
void SvgRenderer::visit(const woml::OccludedFront &theFeature)
{
...
void SvgRenderer::visit(const woml::ParameterValueSetPoint &theFeature)
{
  ...
  // Wind with speed consists of two GeophysicalParameterValues; FlowDirectionMeasure and
  ...
  if (fdm)
  {
    ...
    render_symbol("ParameterValueSetPoint",
                  pointsymbols,
  ...
  if (svm)
  {
    ...
    render_value("ParameterValueSetPoint",
                 pointvalues,
                 theValue.parameter().name(),
                 svm,
                 nullptr,
  ...
  else
  {
    ...
    const woml::NumericalValueRangeMeasure *vrm =
        dynamic_cast<const woml::NumericalValueRangeMeasure *>(theValue.value());
...
std::string PointMeteorologicalSymbolDefinition(const woml::PointMeteorologicalSymbol &theFeature,
                                                std::string &symCode)
{
  const woml::MeteorologicalSymbol symbol = theFeature.symbol();

  woml::MeteorologicalSymbol::DefinitionReferences_const_iterator it =
      symbol.DefinitionReferences_begin();
...
void SvgRenderer::visit(const woml::PointMeteorologicalSymbol &theFeature)
{
  ...
  // Render the symbol
  render_symbol("pointMeteorologicalSymbol",
                pointsymbols,
                symClass,
                symCode,
                theFeature.point()->lon(),
...
RenderPressureCenterTypeDerivedClass(woml::AntiCyclone)
    ...
    void SvgRenderer::visit(const woml::Ridge &theFeature)
{
  ...
  render_front(path, paths, ridges, id, "ridge", "ridgeglyph", "r", "R", fontsize, spacing);
}
...
RenderStormTypeDerivedClass(woml::ConvectiveStorm) RenderStormTypeDerivedClass(woml::Storm)
    ...
    void SvgRenderer::visit(const woml::SurfacePrecipitationArea &theFeature)
{
...
void SvgRenderer::visit(const woml::ParameterValueSetArea &theFeature)
{
  ...
  render_surface(path,
                 precipitationareas,
                 id,
                 theFeature.parameters()->values().front().parameter().name(),
                 &theFeature,
                 &areaSymbols);
}
...
void SvgRenderer::visit(const woml::SurfaceVisibility &theFeature)
{
...
void SvgRenderer::visit(const woml::SurfaceWeather &theFeature)
{
...
void SvgRenderer::visit(const woml::Trough &theFeature)
{
  ...
  render_front(path, paths, troughs, id, "trough", "troughglyph", "t", "T", fontsize, spacing);
}
...
void SvgRenderer::visit(const woml::UpperTrough &theFeature)
{
...
void SvgRenderer::visit(const woml::WarmAdvection &theFeature)
{
...
void SvgRenderer::visit(const woml::WarmFront &theFeature)
{
  ...
  render_front(
      path, paths, warmfronts, id, "warmfront", "warmfrontglyph", Ww, Ww, fontsize, spacing);
}
...
void SvgRenderer::visit(const woml::Winds &theFeature)
{
...
void SvgRenderer::visit(const woml::ZeroTolerance &theFeature)
{
                               ...
                               double defaultValue)
{
  ...
  if (pos1 == std::string::npos || pos1 > pos2) return defaultValue;
  ...
  if (word.substr(word.size() - 2, 2) != "px")
    throw std::runtime_error("CSS value for " + theCssClass + "." + theAttribute +
                             " must be in pixels");
                          ...
                          const boost::posix_time::ptime &theTime)
{
  ...
  if (config.exists("contourlines"))
  {
    ...
    if (!contourSpecs.isList()) throw std::runtime_error(typeMsg);
    ...
    for (int i = 0; i < contourSpecs.getLength(); ++i)
    {
      ...
      if (!specs.isGroup()) throw std::runtime_error(typeMsg);
      ...
      if (!labels.isGroup()) throw std::runtime_error("contourlines labels must be a group");
      ...
      if (labelcoordinates.getLength() % 2 != 0)
        throw std::runtime_error(
            "coordinates setting must have an even number of integer coordinates");

      if (options.verbose)
      {
        std::cerr << "Contourline " << paramname << " class=" << classname << " to " << outputname
                  << " range " << start << "..." << stop << " in step " << step << std::endl;
      }
      ...
      if (matrix.NX() == 0 || matrix.NY() == 0)
        throw std::runtime_error("Could not extract set valid time " +
                                 std::string(validtime.ToStr(kYYMMDDHHMM)) + "from query data");
      ...
      else if (smoother == "Savitzky-Golay")
      {
        ...
        if (options.verbose)
          std::cerr << "Savitzky-Golay smoothing of size " << window << " of degree " << degree
                    << std::endl;
      ...
      for (float value = start; value <= stop; value += step)
      {
        ...
        if (!path.empty())
        {
          ...
          for (int i = 0; i < labelcoordinates.getLength(); i += 2)
          {
            ...
            {
              if (!symbol.empty())
                contours[labeloutputname] << "<use xlink:href=\"#" << symbol << "\" class=\""
                                          << symbolclass << "\" x=\"" << round(x) << "\" y=\""
                                          << round(y) << "\"/>\n";

              contours[labeloutputname] << "<text class=\"" << labelclassname << "\" x=\""
                                        << round(x) << "\" y=\"" << round(y) << "\">" << intvalue
                                        << "</text>\n";
            }
          }
        }
