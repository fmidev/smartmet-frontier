...
//							<< " not found\n";
  ...
  // ----------------------------------------------------------------------
  /*!
   * \brief Move used/reserved 'fillAreas' container's fill areas to
...
//fprintf(stderr,">> %s %.0f,%.0f - %.0f,%.0f [%.0f,%.0f - %.0f,%.0f]\n",eraseReserved ? "erase" : "keep",fiter->first.x,fiter->first.y,fiter->second.x,fiter->second.y,riter->first.x,riter->first.y,riter->second.x,riter->second.y);
   ...
   *		and if 'storeFreeAreas' is true, store the rest free positions to
...
//int TRAP = 0;
	if (!fillAreas.empty()) {
		// Move reserved fill areas to candidate container
		//
